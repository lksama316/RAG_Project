...
class NodeRerank(NodeBase):
    ...
    def _step_2_rerank_merged_docs(self, state: QueryGraphState, merged_multi_docs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 获取文档列表的 content 字段组成列表
            contents = [doc.get("content") for doc in merged_multi_docs if doc.get("content")]
            if not contents:
                return merged_multi_docs  # 无内容则直接返回

            # 调用 Rerank 模型：交叉编码器（精排阶段）
    ...
    def _step_3_cliff_cutoff(self, ranked_docs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        # 遍历范围：从 min_topk-1 到 max_topk-2（索引从0开始），检测相邻两个文档的分数差
